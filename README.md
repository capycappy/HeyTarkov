...
マイクボタンを押している間に**タスク名・マップ名・脱出地点名・KAPPA品名**を言うと、
...
| KAPPA品 | 44 | 「ゴールデンエッグ」 |
| 鍵 | 217 | 「ドーム ルーム サンイチヨン」（鍵ボタン ON のとき） |
...
マイクボタンと検索ボックスの間にある **鍵** ボタンを押すと、鍵だけが出ます。
もう一度押すと元に戻り、鍵は出てこなくなります。

```
鍵ボタン ON   「ドーム ルーム サンイチヨン」 → Dorm room 314 marked key
鍵ボタン OFF  ふだんどおり（タスク・マップ・脱出地点・KAPPA品）
```

- 行にはその鍵が**どのマップのものか**が出ます（`Dorm room 314 marked key / Customs`）
- 左の列はゲームがアイコンに出している短い文字（`Dorm 314`）
- 部屋番号は数字のまま言えます（`314` は「サンイチヨン」でも「サンビャクジュウヨン」でも通ります）
- 鍵ボタンが OFF のときに鍵の名前を言うと、「鍵が◯件あります」と知らせます。ボタンを押せば出ます
